...
const ReceiptsTab = () => {
  ...
  const handleCreateReceipt = (receipt: any) => {
    const newReceipt = {
      ...
      date: receipt.date,
      amount: receipt.items.reduce((sum: number, item: any) => sum + (parseFloat(item.amount) || 0), 0),
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="px-6 py-4 text-left text-sm font-medium text-slate-500">Number</th>
                ...
                <th className="px-6 py-4 text-left text-sm font-medium text-slate-500">Payment Method</th>
              </tr>
            </thead>
            <tbody>
              {sortedReceipts.map(receipt => (
                <tr key={receipt.id} className="hover:bg-slate-50">
                  <td className="px-6 py-4 text-sm text-slate-600">{receipt.number}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">
                    {new Date(receipt.date).toLocaleDateString()}
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-600">${receipt.amount.toFixed(2)}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">
                    {receipt.paymentMethod.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                  </td>
                </tr>
              ))}
            </tbody>
